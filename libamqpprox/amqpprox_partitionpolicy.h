...
class PartitionPolicy {
  public:
    ...
    ///< Modify the partitioning of the specified `backendSet` and return
    ///< a `shared_ptr` to a new `BackendSet` instance with the
    ///< modifications applied.
    ///< When making a connection, a `Server` will attempt all connections
    ///< in the first partition (in an implementation defined order), then
    ///< the second, and so on until all connections have been exhausted.
